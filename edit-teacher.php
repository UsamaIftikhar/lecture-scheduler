...
<?php
try {
    if (isset($_COOKIE['editTeacher'])) {
        $id = $_COOKIE['editTeacher'];
        $fetchUser = $connection->prepare("Select * from user where id='" . $id . "'");
        $fetchUser->execute();
...
try {
    if (isset($_POST['submit'])) {
        $update = $connection->prepare("UPDATE user SET fname='" . $_POST['fname'] . "', lname='" . $_POST['lname'] . "', phoneno='" . $_POST['phoneno'] . "', email='" . $userValues['email'] . "', password='" . crypt($_POST['password'], '$6$rounds=5000$anexamplestringforsalt$') . "' WHERE id='$id'");
        $update->execute();
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="main-card mb-4 card" style="padding-bottom:10px;">
                                <div class="card-header">Teacher Details
                                ...
                                <div style="padding: 30px">
                                    ...
                                    ?>
                                    <form class="needs-validation" novalidate action="edit-teacher.php" method="POST">
                                        ...
                                        <div class="position-relative row form-group"><label for="examplePassword" class="col-sm-2 col-form-label">Email</label>
                                            <div class="col-sm-10"><input name="email" id="email" disabled value="<?PHP echo $userValues['email'] ?>" type="text" class="form-control"></div>
                                        </div>
