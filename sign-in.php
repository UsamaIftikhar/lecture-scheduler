...
try {
    if (isset($_POST['student'])) {
        ...
        $fetchUser = $connection->prepare("Select * from user where email='$email' and password='$pass' and type='student' ");
        $fetchUser->execute();
...
try {
    if (isset($_POST['teacher'])) {
        $email = $_POST['email'];
        $pass = crypt($_POST['password'], '$6$rounds=5000$anexamplestringforsalt$');
        $fetchUser = $connection->prepare("Select * from user where email='$email' and password='$pass' and type='teacher' ");
        $fetchUser->execute();
        $data = $fetchUser->fetch();
...
<body>
    <header class="header-section clearfix">
        ...
        <ul class="main-menu">
            ...
            <li><a style="color: #fc0255;" href="http://localhost:8888/lecture-scheduler/sign-in.php">Login</a>
        </ul>
    </header>
                        ...
                        <div class="card" style="border-radius: 15px; margin-top: 150px; margin-bottom: 150px;">
                            <div class="card-body p-5 ">
                                ...
                                <div class="text-gray-400 fw-bold fs-4 mb-3 d-flex justify-content-center">New Here?&nbsp
                                    <a href="/lecture-scheduler/student-signup.php" class="link-primary fw-bolder">create an account</a>
                                </div>
                                <form method="POST" class="needs-validation" novalidate>
                                    ...
                                    <div class="d-flex justify-content-center">
                                        <button type="student" name="student" id="student" class="btn btn-primary btn-block btn-lg gradient-custom-4 text-body">Continue as Student</button>
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-center">
                                        <button type="teacher" name="teacher" id="teacher" class="btn btn-primary btn-block btn-lg gradient-custom-4 text-body">Continue as Teacher</button>
                                    </div>
                                </form>
                                <script>
